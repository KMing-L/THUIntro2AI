...
def test_batch_size():
    batchSizeList = [4, 8, 16, 32, 64, 128, 256]
    for batchSize in batchSizeList:
        if not os.path.exists('results/result_CNN_bs%d_lr0.010000_ep10_ml64.pickle' % batchSize):
            os.system('python src/main.py --model CNN --batch_size %d' % batchSize)
        if not os.path.exists('results/result_RNN_bs%d_lr0.010000_ep10_ml64.pickle' % batchSize):
            os.system('python src/main.py --model RNN --batch_size %d' % batchSize)
        if not os.path.exists('results/result_MLP_bs%d_lr0.010000_ep10_ml64.pickle' % batchSize):
...
def test_max_length():
    ...
    for maxLength in maxLengthList:
        if not os.path.exists('results/result_CNN_bs16_lr0.010000_ep10_ml%d.pickle' % maxLength):
            os.system('python src/main.py --model CNN --max_length %d' % maxLength)
        if not os.path.exists('results/result_RNN_bs16_lr0.010000_ep10_ml%d.pickle' % maxLength):
            os.system('python src/main.py --model RNN --max_length %d' % maxLength)
        if not os.path.exists('results/result_MLP_bs16_lr0.010000_ep10_ml%d.pickle' % maxLength):
            os.system('python src/main.py --model MLP --max_length %d' % maxLength)

def test_model():
    modelList = ['MLP', 'CNN', 'RNN']
    for model in modelList:
        if not os.path.exists('results/result_%s_bs16_lr0.010000_ep10_ml64.pickle' % model):
